...
The newly obtained TGT can be use like a [golden ticket](golden.md), with [pass-the-ticket](../pass-the/ptt.md), to perform privileged various actions.
...
Once the `.ccache` TGT is obtained, if the attack is successful, the ticket will be usable with [pass-the-ticket](../pass-the/ptt.md). An easy way to check if the TGT works is to use it and ask for a service ticket. This can be done with Impacket's [getST.py](https://github.com/SecureAuthCorp/impacket/blob/master/examples/getST.py) (Python).
...
> [!NOTE]
> In some scenarios, I personally have had trouble using the `.ccache` ticket on UNIX-like systems. What I did was [convert it](../pass-the/ptt.md#practice) to `.kirbi`, switch to a Windows system, inject the ticket with mimikatz's using[`kerberos:ptt`](https://tools.thehacker.recipes/mimikatz/modules/kerberos/pass-the/ptt) command, and then create a new user and add it to the domain admins group.
> 
> ```bash
...
ms14068.exe /domain:$DOMAIN /user:$USER /key:$USER_NThash /sid:$DOMAIN_SID /rid:$USER_RID /kdc:$KDC_FQDN
```
