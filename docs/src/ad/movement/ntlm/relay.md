...
2. An authentication coerce is then triggered (for example, with [PetitPotam](../mitm-and-coerced-authentications/ms-efsr)) from the target to the DNS record, relaying the authentication with [ntlmrelayx](https://github.com/fortra/impacket/blob/master/examples/ntlmrelayx.py) (Python).
...
3. An authentication coercion forces `lsass.exe` to authenticate toward the same share path (e.g. with [PetitPotam](../mitm-and-coerced-authentications/ms-efsr)):
