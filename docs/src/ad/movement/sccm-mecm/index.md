...
* [SMS Provider](https://learn.microsoft.com/en-us/mem/configmgr/core/plan-design/hierarchy/plan-for-the-sms-provider#about): provides a set of interfaces between the site server and the site database. It exposes WMI and the AdminService REST API to admin consoles and tools (such as the Configuration Manager console) to read and manage site data. Note that clients do not communicate with the SMS Provider — they communicate with the Management Point instead. "SMS" stands for "System Management Service".
