...
const currentSponsor = computed(() => {
  return data.value?.find(sponsor => sponsor.tier === 'Banner Sponsors')
    ?.items.find(banner => banner.categories.includes(currentCategory.value));
});

const theme = computed(() => {
  if (!currentSponsor.value) {
    return { primaryColor: '#000', secondaryColor: '#000', logo: '' };
  }
...
const gradientBorder = computed(() => `linear-gradient(120deg, ${theme.value.secondaryColor}, ${theme.value.primaryColor})`);
...
.banner-sponsor:hover {
  ...
  box-shadow: 0 0 10px v-bind('theme.primaryColor'), 0 0 20px v-bind('theme.secondaryColor');
